...
// Pre-cache the caustics images.
// Without this, the first image sometimes used to start with 100% opacity.  (That happened if it was added to the scene before it had finished loading.)
cc.TextureCache.getInstance().addImageAsync('Resource/lcaustics1_tiled.png');
cc.TextureCache.getInstance().addImageAsync('Resource/lcaustics2_tiled.png');
...
var GameBackgroundLayer = cc.Layer.extend({
    ...
    initCaustics:function () {
        var numberOfCaustics = 3;
        var maxOpacity = 40;
        // Increase this to slow down the motion
        var movementDuration = 2.5;
        // Increase this to slow the change between caustics (this will slow the fade speed)
        var secondsBetweenCaustics = 1.0;

        var scrWidth = VisibleRect.rect().size.width;
        var scrHeight = VisibleRect.rect().size.height;

        // Each caustic will spend half its time fading in, and half fading out
        var secondsToFade = secondsBetweenCaustics * numberOfCaustics / 2;

        for (var i = 0; i < numberOfCaustics; i++) {
            var img = 'lcaustics' + (1 + (i % 2)) + '_tiled.png';
            //img = img + '?nocache=' + Date.now();
            var sprite = cc.Sprite.create(ImageName(img));

            // We want each caustic to start a bit later than the previous one
            // (I didn't find a way to delay the RepeatForever sequences, so we just setTimeout)
            setTimeout(startCaustic.bind(this, sprite, i), secondsBetweenCaustics * i * 1000);
        }

        function startCaustic (sprite, i) {
            this.addChild(sprite);
            this._caustics.push(sprite);

            var sign = ((i+1) % 4) < 2 ? -1 : +1;

            sprite.setPosition(scrWidth * (0.5 + 0.01 * sign), scrHeight * (0.5 + 0.3 * sign));

            sprite.setScale(1.0 + 0.10 * i);
            if (i < 2) {
                // When the size is still small, we can only do 90 degree rotations, or the edge of the sprite will be visible!
                sprite.setRotation(90 * Math.floor(i / 2));
            } else {
                // But for the later sprites which have larger scales, we can rotate arbitrarily
                sprite.setRotation(37 * i);
            }

            sprite.setSkewY(-2 * sign);
            sprite.setOpacity(0);

            var moveup = cc.MoveBy.create(1.5 * movementDuration, cc.p(0, 3 * i));
            ...
            var skewup = cc.SkewBy.create((1.3 + 0.1 * i) * movementDuration, 1 * sign, 2 * sign);
            ...
            var scaleup = cc.ScaleBy.create((1.1 - 0.1 * i) * movementDuration, .99, 1);
            ...
            var fadeIn = cc.EaseInOut.create(cc.FadeTo.create(secondsToFade, maxOpacity), 2);
            var fadeOut = cc.EaseInOut.create(cc.FadeTo.create(secondsToFade, 0), 2);
            var fadeInAndOut = cc.Sequence.create(fadeIn, fadeOut);
            var repeat4 = cc.RepeatForever.create(fadeInAndOut);

            sprite.runAction(repeat);
            sprite.runAction(repeat2);
            sprite.runAction(repeat3);
            sprite.runAction(repeat4);
        }

        // Possible improvements:
        // When a caustic is faded out to opacity 0, we can change its rotation, flip, scale, motion, etc. so it will
        // appear different when it fades back in.
    },
    haibeiAni:function () {
        var frames = [];
