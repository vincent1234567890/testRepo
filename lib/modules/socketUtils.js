    ...
    function createIOSocket (webSocket) {
        ...
        const ioSocket = {
            ...
            emit: function (event, data) {
                // data should be an object, so we can put `_` inside it.
                // If we were passed undefined or null, then we will use an empty object.
                if (data == null) {
                    data = {};
                }
                if (typeof data !== 'object') {
                    throw Error("socketUtils.emit(event, data) expects data to be an object (or undefined)");
                }
