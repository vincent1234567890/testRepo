...
(function(root, factory) {
    ...
    function createIODataHandler (emitter) {
        const handler = function (data, callIfService) {
            // Tracking a bug:
            if (callIfService !== undefined && typeof callIfService !== 'function') {
                console.warn("Passed non-function callIfService:", callIfService, Error().stack);
            }
            // Sometimes events.js `emitTwo` was calling us, passing a buffer as the second argument.
            // But now we fixed this, so the above check is no longer needed.
            ...
            if (typeof data === 'string' && data[0] === '{' && data[1] === '"') {
                ...
                if (isNode) {
                    try {
                        ...
                        // Unfortunately the packet may have lost its _ property by now, so although 'packet' has the data, we don't know the action.
