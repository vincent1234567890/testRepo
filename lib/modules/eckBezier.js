    ...
    function createBezierCurveEditor (cubicCanvas, options) {
        ...
        function cubicDraw () {
            ...
            if (options.showControls) {
                cubicGraphics.lineWidth = 1;
                if (lockPairs) {
                    cubicGraphics.strokeStyle = "#0000cc";
                }
                else {
                    cubicGraphics.strokeStyle = "#0000cc";
                }
                if (!options.editable) {
                    cubicGraphics.strokeStyle = 'grey';
                }
                cubicGraphics.save();
                ...
                for (let i = 0; i < cubicPoints.length; i++) {
                    ...
                    if (i % 3 == 0) {
                        ...
                        if (!options.editable) {
                            cubicGraphics.fillStyle = 'grey';
                        }
                        disk(cubicGraphics, cubicPoints[i].x, cubicPoints[i].y, handleRadius);
                        if (isStartOrEndPoint) {
                            const dirX = cubicPoints[i].x > 20 ? -1 : +1;
                            const dirY = cubicPoints[i].y > 20 ? -1 : +1;
                            cubicGraphics.font = '12px sans-serif';
                            cubicGraphics.textAlign = 'center';
                            cubicGraphics.textBaseline = 'middle';
                            cubicGraphics.fillText('' + (Math.floor(i / 3) + 1), cubicPoints[i].x + 12 * dirX, cubicPoints[i].y + 12 * dirY);
                        }
                    }
                    else {
                        cubicGraphics.fillStyle = options.editable ? "blue" : "grey";
        ...
        function doCubicMouseDown (evt) {
            if (draggingCubic || !options.showControls || !options.editable) {
                return;
            }
        ...
        function doCubicDoubleClick (evt) {
            if (!options.showControls || !options.editable) {
                return;
            }
            const clickedPointIndex = findClickedPoint(evt);
            if (clickedPointIndex > 0 && clickedPointIndex < cubicPoints.length - 1 && clickedPointIndex % 3 === 0) {
                ...
                if (positionOnPath) {
                    ...
                    cubicPoints.splice(i + 2, 0,
                        addVector(clickedPoint, multiplyVector(dirVector, -0.25 * avgDistance)),
                        clickedPoint,
                        addVector(clickedPoint, multiplyVector(dirVector, +0.25 * avgDistance))
                    );
                    //doLock();
        ...
        document.addEventListener("mouseup", doCubicMouseUp, false);
        ...
        function checkCleanup () {
            if (!isInDocument(cubicCanvas)) {
                ...
                document.removeEventListener("mouseup", doCubicMouseUp);
