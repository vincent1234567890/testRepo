...
(function(root, factory) {
    ...
    const serverReceiver = function (player, ioSocket, fishGame, arena) {
        ...
        ioSocket.on('b', data => {
            ...
            arena.spawnBullet(player, bulletId, player.gunId, angle, now);
            player.lastShootTime = now;

            // In future we might inform all clients *except* 'player', and let him immediately add the bullet to his client arena.
            fishGame.informAllClients.playerFiredBullet(player.playerId, bulletId, player.gunId, angle, arena.getGameTime());
        });
