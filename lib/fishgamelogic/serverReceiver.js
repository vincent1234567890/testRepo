...
(function(root, factory) {
    ...
    const serverReceiver = function (player, ioSocket, fishGame, arena) {
        ...
        ioSocket.on('g', data => {
            ...
            fishGame.informAllClientsExcept(player).playerSelectedGun(player.playerId, gunId);
        });
        ...
        ioSocket.on('p', data => {
            ...
            fishGame.informAllClientsExcept(player).playerPointedGun(player.playerId, angle);
        });
        ...
        ioSocket.on('b', data => {
            ...
            // Dock player credits
            arena.changePlayerScore(arenaPlayer.playerId, -bulletCost);

            arena.spawnBullet(player.playerId, bulletId, arenaPlayer.gunId, angle, now);
            player.lastShootTime = now;

            // In future we might inform all clients *except* 'player', and let him immediately add the bullet to his client arena.
            fishGame.informAllClients.playerFiredBullet(player.playerId, bulletId, arenaPlayer.gunId, angle, arena.getGameTime());
        });
