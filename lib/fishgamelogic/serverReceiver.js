...
(function(root, factory) {
    ...
    const serverReceiver = function (player, ioSocket, fishingRoom, arena) {
        ...
        ioSocket.on('g', data => {
            ...
            fishingRoom.informAllClientsExcept(player).playerSelectedGun(player.id, gunId);
        ...
        ioSocket.on('p', data => {
            ...
            fishingRoom.informAllClientsExcept(player).playerPointedGun(player.id, angle);
        ...
        ioSocket.on('b', data => {
            ...
            if (arenaPlayer.score < bulletCost) {
                ...
                player.informClient.error("You cannot afford to fire this bullet.");
                return;
            }

            if (arenaPlayer.bulletsOnScreen >= gameConfig.maxBulletsPerPlayer) {
                player.informClient.error("Maximum bullets exceeded.");
                return;
            }
            ...
            fishingRoom.informAllClients.playerFiredBullet(player.id, bulletId, arenaPlayer.gunId, angle, now);
        ...
        ioSocket.on('leaveGame', () => {
            fishingRoom.removePlayer(player);
        });
