...
(function(root, factory) {
    ...
    const clientReceiver = function (ioSocket, gameCtrl) {
        ...
        ioSocket.on('J', data => {
            ...
            // We don't bother to store the slot or the cannonPosition.
            // We can retrieve these when we need from arena.getPlayer(myPlayerId)
            //const cannonPosition = gameConfig.cannonPositions[playerSlot];

            gameConfig = config;
            gameCtrl.setGameConfig(config);

            arena = new FishGameArena(false, gameConfig);
        ...
        ioSocket.on('gs', gameState => {
            ...
            for (let fishId in fishes) {
                ...
                arena.addFish(fish.type, fish.fishId, fish.position, fish.angle, fish.motionPatternGroup, fish.motionPatternId, fish.motionPatternBit, fish.bitStartTime, fish.health);
                createFishActor(fish.type, fish.fishId, fish.position);
            }
