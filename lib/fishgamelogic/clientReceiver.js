...
(function (root, factory) {
    ...
    const clientReceiver = function (ioSocket) {
        ...
        ioSocket.on('J', data => {
            ...
            const spectating = data.S;   // Are you playing as a spectator?
            ...
            GameManager.isSpectator = spectating;
            GameManager.isPlayer = !spectating;
        ...
        ioSocket.on('b', data => {
            ...
            // When we receive a bullet, we want the bullet's creation time to match our local arena's game time.
            // Thus the client arena always runs X seconds behind the server arena (where X is the network delay between server send and client receipt.)
            // This is preferable to running our arena on server time, because that would cause bullets to appear far in front of the cannons!
            // Our delay is initially calculated when we first join the game.
            // But network congestion might cause the delay by which we receive packets to increase or decrease.
            // The following code will adjust the local arena time if we notice the packets are arriving later or earlier than expected.
            // @todo Unfortunately it also gets triggered when Chrome focuses back on a tab which has been idle (hidden), because Chrome pauses+queues network packets for unfocused tabs.
            // This can result in heavy delay being introduced when it isn't really needed, and then a slow return to normal speed.
            ...
            if ((delta < -200 || delta > +50) && Math.random() < 0.5) {
                const adjustment = delta < 0 ? Math.max(-400, delta) : Math.min(+50, delta);
                console.info("Lag / decongestion detected (" + delta + ").");
                if (shiftGameTimeWhenLagging) {
                    console.info("Shifting game time by " + adjustment + "ms for consistent view.");
        ...
        ioSocket.on('playerChangedSeat', data => {
            ...
            // arenaPlayer does not contain the locking state, so the state will be lost when we switch to the new view.
            // So we will store the state here, and restore it manually afterwards.
            const currentPlayerLockMode = ef.gameController.getLockMode();

            GameManager.clearPlayerState(oldSlot);
            GameManager.updateMultiplayerState(arenaPlayer, oldSlot);

            if (GameManager.isCurrentPlayer(arenaPlayer.id)) {
                GameView.setPlayerLockStatus(newSlot, currentPlayerLockMode);
            }
        });
        ...
        //        priority: 1,
        //        message: `${data.playerName} has won the ${data.jackpotName} jackpot worth ${data.value}!`,
        //    });
        //});
        ...
        ioSocket.on('psjg', data => {
            ...
            const seatNo = ef.gameController.getCurrentSeat();
            const matr = GameManager.getGameConfig().cannonPositions[seatNo];

            if (!cc.spriteFrameCache.getSpriteFrame(ReferenceName.JackpotCannonLight)) {
                cc.spriteFrameCache.addSpriteFrames(res.JackpotMiniGame2Plist);
            }
            const jpLight = new cc.Sprite(ReferenceName.JackpotCannonLight);
            const jpNotification = new cc.Sprite(ReferenceName.JackpotPlayerNotificationBackground);
            jpNotification.setPosition(matr[0], matr[1]);
            jpNotification.setScale(0.001, 0.001);

            const lbPlayer = new cc.LabelTTF(ef.gameController.getTranslatedText('jpWinnerTxt')+": " + data.j.name, "Arial", 20);
            let notiContent = jpNotification.getContentSize();
            lbPlayer.setPosition(notiContent.width / 2, notiContent.height / 2);
            jpNotification.addChild(lbPlayer);


            const offset = 130;
            const lightDuration = 2.5;
            switch (seatNo) {
                case 0:
                case 1:
                    jpLight.setPosition(matr[0], matr[1] + offset);
                    break;
                case 2:
                    jpLight.setPosition(matr[0] + offset, matr[1]);
                    jpLight.setRotation(90);
                    break;
                case 3:
                    jpLight.setPosition(matr[0] - offset, matr[1]);
                    jpLight.setRotation(-90);
            ...
            const duration = 1;
            const moveToCenter = cc.moveTo(2, cc.p(cc.winSize.width * 0.5, cc.winSize.height * 0.5)).easing(cc.easeOut(duration));
            const scaleTo1 = cc.scaleTo(1, 1).easing(cc.easeOut(duration));
            const spawnNoti = cc.spawn(scaleTo1, moveToCenter);
            jpLight.setOpacity(0);
            watchingJackpotPanel = new JackpotPanel(isMyGame, jackpotRewardObject, onFinish);

            GameView.addView(jpLight, 1);
            GameView.addView(jpNotification, 1);
            jpLight.runAction(cc.sequence(
                cc.fadeIn(lightDuration),
                cc.delayTime(0.5),
                new cc.CallFunc(function () {
                    lbPlayer.runAction(cc.fadeOut(1));
                }),
                cc.delayTime(1),
                cc.fadeOut(1),
                new cc.CallFunc(function () {
                    GameView.destroyView(jpLight);
                    GameView.addView(watchingJackpotPanel, 10);
                })
            ));

            jpNotification.runAction(cc.sequence(
                spawnNoti,
                cc.delayTime(1),
                cc.fadeOut(1),
                new cc.CallFunc(function () {
                    GameView.destroyView(jpNotification);
                })
            ));
        ...
        ioSocket.on('youCanNowLockTheRoom', (lockingConfig) => {
            // @todo Replace this confirmation dialog with a pretty game panel

            // const wantsToLock = confirm(`You can now lock this room! It will cost ${lockingConfig.costToLockRoom} for ${lockingConfig.initialLockMinutes} minutes. Do you want to lock the room?`);

            const touchEater = () => {};

            const lockRoom = new ef.LockingRoomDialog(lockingConfig.costToLockRoom, lockingConfig.initialLockMinutes, function (ans) {
                BlockingManager.deregisterBlock(touchEater);
                ClientServerConnect.getServerInformer().requestLockGame(ans);
            });
            lockRoom.setPosition(cc.winSize.width * 0.5, cc.winSize.height * 0.5);
            GameView.addView(lockRoom, 100);
