    ...
    const clientReceiver = function (ioSocket, gameCtrl) {
        ...
        ioSocket.on('b', data => {
            ...
            if (delta < -20 || delta > 1000) {
                console.log(`${Date()} Received a new bullet ${bulletId} late by ${delta}ms`);
            }
    ...
    clientReceiver.ghostActors = function (ioSocket, ms) {
        if (GameCtrl.debugGhosts) {
            ...
            ioSocket._emitter.emit = function () {
                ...
                // Note that if we give lag to J/gs events
                const lagMs = type === 'J' || type === 'gs' ? delayForInitialPackets
                ...
                if (type === 'f' || type === 'b' || type === 'l' || type === 'x' || type === 'k') {
                    ...
                    try {
                        ...
                        // It is quite normal for 'l' events to fail for early ghost fish, because 'gs' never added them!
                        console.warn(e);
                    }
                }
