...
(function(root, factory) {
    ...
    const clientReceiver = function (ioSocket, gameCtrl) {
        ...
        ioSocket.on('gs', gameState => {
            ...
            const gameStartTime = Date.now() - gameState.arenaState.gameTime;

            //console.log("Received gameState:", gameState);

            arena = new FishGameArena(false, gameConfig, null, gameStartTime);
            ...
            for (let fishId in fishes) {
                ...
                arena.addFish(fish.type, fish.id, fish.position, fish.angle, fish.motionPatternGroup, fish.motionPatternId, fish.motionPatternBit, fish.bitStartTime, fish.health);
                createFishActor(fish.type, fish.id, fish.position);
            }
            for (let bulletId in bullets) {
                const bullet = bullets[bulletId];
                arena.addBullet(bullet.ownerId, bullet.id, bullet.gunId, bullet.position, bullet.velocity);
        ...
        ioSocket.on('b', data => {
            ...
            const shiftGameTimeWhenLagging = false;   // option
            if (shiftGameTimeWhenLagging) {
                // If delta is consistently out-of-sync, then we should adjust our local game's delay accordingly.
                // @todo Only shift if delta is *consistent*.  We need a consistency detection algorithm!  (Windowed average?)
                //   arena.checkDelta(delta);
                if ((delta < -500 || delta > +500) && Math.random() < 0.1) {
                    console.error("");
                    console.error("Lag detected!  Shifting game time for consistent view, by " + delta + "ms");
                    console.error("");
                    // This will cause fish to pause for a moment (positive) or speed up for a moment (negative)
                    // We should consider spreading it out over many small increments
                    arena.shiftGameTime(delta);
                }
            }
            ...
            if (arena.hasPlayer(ownerId)) {
                ...
                arena.changePlayerScore(ownerId, -gunClass.bulletCost);
                const player = arena.getPlayer(ownerId);
                GameManager.updateMultiplayerState(player);
                ...
                createBulletActor(ownerId, bulletId, angle, showCannonAnimation);
            }
        });
        ...
        ioSocket.on('f', data => {
            ...
            createFishActor(fishType, fishId, position);
        });
        ...
        // Probably deprecated.  Use the service action 'getMyStats' instead.
        ...
        ioSocket.on('kickedFromGame', data => {
            ...
            console.warn("Not yet implemented: kickedFromGame because " + reason);
        });
        ...
        function createFishActor (fishType, fishId, position) {
            ...
            //for debugging
            //to be removed
            //nooooo don't ever remove it!
            if (GameManager.debug) {
                var debugCircle = new cc.Sprite(res.DebugCircle);
                var config = GameManager.getGameConfig();
                var fishClass = config.fishClasses[fishType];
                var additionalMultiplier = 1;
                var data = FishAnimationData[fishType];

                debugCircle.setScaleX(fishClass.length * 2 * additionalMultiplier / 100);
                debugCircle.setScaleY(fishClass.breadth * 2 * additionalMultiplier / 100);

                fishActor.addChild(debugCircle, 1);
                //console.log(fishActor);
            }

            if (GameCtrl.debugGhosts && fishId.match(/_ghost$/)) {
        ...
        function createBulletActor (ownerId, bulletId, angle, showCannonAnimation) {
            ...
            // Eugene :
            // weaponManager now deprecated (probably)
            // @todo Get the cannon for the appropriate player
            // const weaponManager = GameCtrl.sharedGame().getCurScene().getCannonActor();
            // if (!weaponManager) {
            //     console.warn("weaponManager is null.  This sometimes happens just at game start.  Cannot create this BulletActor.")
            //     return;
            // }
            ...
            const bulletActor = GameManager.shootTo(ownerId, tempTarget);
            bulletActor.bulletId = bulletId;

            if (GameCtrl.debugGhosts && bulletId.match(/_ghost$/)) {
                bulletActor.setOpacity(80);
            }

            bulletActorsByBulletId[bulletId] = bulletActor;
        }

        function createPlayerActor (playerId, playerName, playerSlot) {
    ...
    clientReceiver.ghostActors = function (ioSocket, ms) {
        if (GameCtrl.debugGhosts) {
            ...
            ioSocket._emitter.emit = function () {
                ...
                const lagMs = type === 'J' || type === 'gs' || !GameCtrl.sharedGame().getArena() ? delayForInitialPackets
