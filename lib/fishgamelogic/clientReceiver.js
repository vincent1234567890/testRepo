...
(function(root, factory) {
    ...
    const clientReceiver = function (ioSocket) {
        ...
        ioSocket.on('b', data => {
            ...
            if (arena.hasPlayer(ownerId)) {
                arena.addBullet(ownerId, bulletId, gunId, bulletCost, angle, startTime);
        ...
        ioSocket.on('k', data => {
            ...
            for (let fishCaughtEvent of fishCaughtEvents) {
                ...
                const playerSlot = arena.getPlayer(playerId).slot;
        ...
        ioSocket.on('youWonJackpot', data => {
            ...
            // Claim the jackpot
            ClientServerConnect.getGameWSClient().callAPIOnce('game', 'collectJackpot', {rewardLogObjId: data.rewardLogObjId}).then(
                response => {
        ...
        ioSocket.on('playerChangedSeat', data => {
            ...
            // @todo Test this
            const arenaPlayer = arena.getPlayers().find(p => p.slot === oldSlot);
            arenaPlayer.slot = newSlot;

            // @todo Update other relevant things on the front-end, e.g. the view

            //GameManager.updateMultiplayerState(???);
        ...
        ioSocket.on('rc', data => {
            ...
            const roundName = data.n;

            arena.setRoundNumber(roundNumber);
            arena.setRoundName(roundName);
        });
    ...
    function createArenaFromState (gameConfig, gameState, gameStartTime, serverGameTime) {
        ...
        arena.setRoundName(gameState.arenaState.roundName);
