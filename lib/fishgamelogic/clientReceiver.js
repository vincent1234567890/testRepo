    ...
    const clientReceiver = function (ioSocket, gameCtrl) {
        ...
        ioSocket.on('gs', gameState => {
            ...
            for (let fishId in fishes) {
                ...
                arena.addFish(fish.type, fish.id, fish.position, fish.angle, fish.motionPatternGroup, fish.motionPatternId, fish.motionPatternBit, fish.bitStartTime, fish.health, gameState.arenaState.gameTime);
                createFishActor(fish.type, fish.id, fish.position);
            }
            for (let bulletId in bullets) {
                const bullet = bullets[bulletId];
                arena.addBullet(bullet.ownerId, bullet.id, bullet.gunId, bullet.position, bullet.velocity, gameState.arenaState.gameTime);
        ...
        ioSocket.on('b', data => {
            ...
            if (shiftGameTimeWhenLagging) {
                ...
                if ((delta < -250 || delta > +50) && Math.random() < 0.5) {
                    const adjustment = delta < 0 ? -50 : +50;
        ...
        function createBulletActor (ownerId, bulletId, angle, showCannonAnimation) {
            ...
            const cannonPosition = arenaPlayer && arenaPlayer.cannonPosition || [0, 0];
