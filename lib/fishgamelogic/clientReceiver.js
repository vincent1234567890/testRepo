...
(function(root, factory) {
    ...
    const clientReceiver = function (ioSocket) {
        ...
        //receive the error from server, we should show the error message more friendly.
        ...
        //player change the angle of his gun.
        ioSocket.on('a', data => {
        ...
        // current player won the jackpot event.
        ioSocket.on('youWonJackpot', data => {
            ...
            ClientServerConnect.listUncollectedJackpots().then(response => {
                ...
                // Choose which one to play now (the first one)
                const jackpotRewardLog = jackpotRewardLogs[0];
                console.log("jackpotRewardLog:", jackpotRewardLog);

                const jackpotPanel = new JackpotPanel(true, jackpotRewardLog);
                GameView.addView(jackpotPanel,10);

                ClientServerConnect.getServerInformer().shareJackpotGameStarted(jackpotRewardLog.jackpotId);
            }).catch(console.error);
        });
        ...
        ioSocket.on('psjg', data => {
            ...
            //alert("Someone else is playing jackpot game!");

            watchingJackpotPanel = new JackpotPanel(false, jackpotRewardObject);
        ...
        //todo should be move to game manager
