        ...
        function handleCollision (bullet, collisionFish, explosionPosition) {
            ...
            for (let fishId in fishes) {
                ...
                if (distanceSquared < maxDist * maxDist) {
                    ...
                    const damageDealt = Math.round(damagePoints * penetrationIntoFish);

                    if (damageDealt > 0) {
                        fish.health -= damageDealt;

                        if (options.logCollisions) {
                            console.log(`x Bullet ${bullet.id} hit fish ${fish.id} with damage ${damageDealt}`);
                        }
                        ...
                        if (fish.health <= 0) {
                            ...
                            if (player) {
                                ...
                                // @todo Send score or scoreChange to DB, either debounced or queued respectively
                            }
