...
(function(root, factory) {
    ...
    function FishGameArena (onServer, gameConfig, fishGame, _gameStartTime) {
        ...
        function spawnFish (fishType, fishId, position, angle, fishSpawnTime, motionPatternGroup, motionPatternId, fishHealth) {
            const fishClass = getFishClass(fishType);

            return addFish(fishType, fishId, position, angle, motionPatternGroup, motionPatternId, 0, fishSpawnTime, fishHealth, fishSpawnTime);
        }

        function addFish (fishType, fishId, position, angle, motionPatternGroup, motionPatternId, motionPatternBit, bitStartTime, fishHealth, lastUpdateTime) {
            const fish = {
                type: fishType,
                id: fishId,
                position: position,
        ...
        function moveFish (fish, deltaTime) {
            ...
            */
            ...
            // @todo Scale relative to fish speed
            const timeSinceSpawn = currentTime - fish.bitStartTime;
            let tPath = timeSinceSpawn / motionPattern.duration / 1000;
            fish.tPath = tPath;
            if (tPath < 0) {
                tPath = 0;
            } else if (tPath >= 1) {
            ...
            const whichCurve = Math.floor(tPath * 3);
            const tCurve = tPath * 3 - whichCurve;

            const points = motionPattern.points;
            const i = whichCurve * 3;

            // @todo Re-use p object?
