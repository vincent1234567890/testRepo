...
(function(root, factory) {
    ...
    const FIRST_GOLDEN_FISH_TIER = 100;
    ...
    function FishGameArena (onServer, gameConfig, fishingRoom, _gameStartTime, _consumptionInThisGame) {
        ...
        let dateWhenRoundChanged = new Date();
        ...
        function addFish (fishType, fishId, offsetFromGroup, angle, motionPatternGroup, motionPatternId, motionPatternBit, fishSpawnTime, fishHealth, lastUpdateTime, fishSpeed) {
            ...
            //should create a fish entity class.
            const fish = {
                type: fishType,
                id: fishId,
                position: [-1000, -1000],
            ...
            if (fishClass.tier >= FIRST_GOLDEN_FISH_TIER) {
                numberOfGoldFish++;
            }
        ...
        function addBullet (ownerId, bulletId, gunId, bulletCost, angle, startTime, targetLockFishId) {
            ...
            let position;
            if (arenaPlayer) {
                position = arenaPlayer.cannonPosition.slice(0);
        ...
        function moveBullet (bullet, deltaTime) {
            ...
            if (bullet.targetFish) {
                ...
                const fishIsVisible = targetFish && fishIsOnScreen(targetFish);
                if (targetFish && fishIsVisible) {
        ...
        function isFishOffScreen (fish) {
            const fishPos = fish.position;
            const fishClass = getFishClass(fish.type);
            const fishRadius = Math.max(fishClass.length, fishClass.breadth);
            if (fishPos[0] < gameConfig.ARENA_LEFT - fishRadius) {
                return true;
            }
            if (fishPos[0] > gameConfig.ARENA_RIGHT + fishRadius) {
                return true;
            }
            if (fishPos[1] < gameConfig.ARENA_BOTTOM - fishRadius) {
                return true;
            }
            if (fishPos[1] > gameConfig.ARENA_TOP + fishRadius) {
                return true;
            }
        ...
        function fishIsOnScreen (fish) {
            const fishClass = fish && getFishClass(fish.type);
            return isEllipseInsideRectangle(fish.position, fishClass.length, fishClass.breadth, fish.angle, gameConfig.ARENA_LEFT, gameConfig.ARENA_RIGHT, gameConfig.ARENA_BOTTOM, gameConfig.ARENA_TOP);
        }
        ...
        function moveFish (fish, deltaTime) {
            ...
            const fishAreScattering = gameTimeWhenModeChanged >= 0 && (currentArenaMode === 'clearing_for_free_shooting_game' || currentArenaMode === 'clearing_for_round_change');
        ...
        function markAsDictionaryNotArray (obj) {
            Object.defineProperty(obj, 'length', {
                get: function () {
                    ...
                    //console.log(`Making inefficient access to dictionary.length!`);
        ...
        function gameHasUnpaused (pauseDuration) {
            gameIsPaused = false;
            // Update fishes and bullets so they won't leap ahead on the next tick
            for (let fishId in fishes) {
                const fish = fishes[fishId];
                fish.lastUpdateTime += pauseDuration;
                fish.spawnTime += pauseDuration;
            }
            for (let bulletId in bullets) {
                const bullet = bullets[bulletId];
                bullet.lastUpdateTime += pauseDuration;
            }
            gameTimeWhenModeChanged += pauseDuration;
        }
        ...
        return {
            ...
            incrementRoundNumber: () => {
                ++currentRoundNumber;
                dateWhenRoundChanged = new Date();
            },
            getDateWhenRoundChanged: () => dateWhenRoundChanged,
            ...
            clearConsumption: () => consumptionInThisGame = 0,
            ...
            isFishOffScreen: isFishOffScreen,
            fishIsOnScreen: fishIsOnScreen,
            ...
            gameHasUnpaused: gameHasUnpaused,
        };
    }
    ...
    function isEllipseInsideRectangle (ellipseCenter, ellipseWidthRadius, ellipseHeightRadius, ellipseAngle, rectLeft, rectRight, rectBottom, rectTop) {
        // As a quick approximation, we will pretend the ellipse is a rhombus, and just check the extreme points
        const point1 = [ellipseCenter[0] + ellipseWidthRadius * Math.cos(ellipseAngle), ellipseCenter[1] + ellipseWidthRadius * Math.sin(ellipseAngle)];
        const point2 = [ellipseCenter[0] - ellipseWidthRadius * Math.cos(ellipseAngle), ellipseCenter[1] - ellipseWidthRadius * Math.sin(ellipseAngle)];
        const point3 = [ellipseCenter[0] + ellipseHeightRadius * Math.sin(ellipseAngle), ellipseCenter[1] - ellipseHeightRadius * Math.cos(ellipseAngle)];
        const point4 = [ellipseCenter[0] - ellipseHeightRadius * Math.sin(ellipseAngle), ellipseCenter[1] + ellipseHeightRadius * Math.cos(ellipseAngle)];
        return (false
            || isPointInsideRectangle(point1, rectLeft, rectRight, rectBottom, rectTop)
            || isPointInsideRectangle(point2, rectLeft, rectRight, rectBottom, rectTop)
            || isPointInsideRectangle(point3, rectLeft, rectRight, rectBottom, rectTop)
            || isPointInsideRectangle(point4, rectLeft, rectRight, rectBottom, rectTop)
        );
    }

    function isPointInsideRectangle (point, left, right, bottom, top) {
