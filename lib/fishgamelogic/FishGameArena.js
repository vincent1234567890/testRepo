    ...
    function FishGameArena (onServer, gameConfig, fishGame, _gameStartTime) {
        ...
        function considerSpawningFish () {
            if (fishes.length < gameConfig.maxFishOnScreen) {
                ...
                const spacing = 2.5 * fishClass.breadth;
                ...
                for (let i = 0; i < numInGroup; i++) {
                    ...
                    const position = [-fishClass.length - 20, centerOfSpawnY + spacing * (i - numInGroup/2 + 0.5)];
            ...
            for (let bulletId in bullets) {
                ...
                for (let fishId in fishes) {
                    ...
                    if (distanceSquared < maxDist * maxDist) {
                        ...
                        const angleToBullet = Math.atan2(bullet.position[1] - fish.position[1], bullet.position[0] - fish.position[0]);
                        ...
                        const normalisedDistanceFromCenter = Math.pow(vectorToBulletEdgeInFishSpace[0] / fishLength, 2) + Math.pow(vectorToBulletEdgeInFishSpace[1] / fishBreadth, 2);
