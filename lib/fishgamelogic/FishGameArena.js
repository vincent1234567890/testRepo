            ...
            for (let bulletId in bullets) {
                ...
                for (let fishId in fishes) {
                    ...
                    if (distanceSquared < maxDist * maxDist) {
                        ...
                        // Note that this is an approximation:  We step from the center of the bullet towards the center of the ellipse, which might not actually be best direction to the surface of the ellipse.  This means the collision may fail if a bullet skims over the cheek of a fish which is long and thin.
                        const distanceBetweenCenters = Math.sqrt(distanceSquared);
                        const _distanceToBulletEdge = distanceBetweenCenters - bulletRadius;
                        const distanceToBulletEdge = _distanceToBulletEdge >= 0 ? _distanceToBulletEdge : 0;
                        const vectorToBullet = [ bullet.position[0] - fish.position[0], bullet.position[1] - fish.position[1] ];
                        const angleToBullet = Math.atan2(vectorToBullet[1], vectorToBullet[0]);

                        const angleToBulletInFishSpace = angleToBullet - fish.angle;
                        const vectorToBulletEdgeInFishSpace = [distanceToBulletEdge * Math.cos(angleToBulletInFishSpace), distanceToBulletEdge * Math.sin(angleToBulletInFishSpace)];

                        const normalisedDistanceFromCenterSquared = Math.pow(vectorToBulletEdgeInFishSpace[0] / fishLength, 2) + Math.pow(vectorToBulletEdgeInFishSpace[1] / fishBreadth, 2);
                        const vectorIsInsideEllipse = normalisedDistanceFromCenterSquared < 1;

                        if (vectorIsInsideEllipse) {
                            // Collision!

                            //const collisionPosition = bullet.position;

                            // We want to explode on the edge of the ellipse, not inside it.
                            // So let's determine exactly where the bullet hit the ellipse.

                            // Push from the vectorToBulletEdgeInFishSpace to the edge of the ellipse, should work in world space too.
                            const scalarToPushToEdgeOfEllipse = 1 / Math.sqrt(normalisedDistanceFromCenterSquared);

                            // Method 1: Scale in fish space, then move to world space
                            //const vectorToCollisionInFishSpace = vectorToBulletEdgeInFishSpace.map(c => c * scalarToPushToEdgeOfEllipse);
                            //const collisionX = fish.position[0] + vectorToCollisionInFishSpace[0] * Math.cos(fish.angle) - vectorToCollisionInFishSpace[1] * Math.sin(fish.angle);
                            //const collisionY = fish.position[1] + vectorToCollisionInFishSpace[1] * Math.cos(fish.angle) + vectorToCollisionInFishSpace[0] * Math.sin(fish.angle);
                            //const collisionPosition = [collisionX, collisionY];

                            // Method 2: In world space, scale from the center of the bullet to the edge of the bullet, and then push out again
                            const scalarToShrinkToEdgeOfEllipse = (distanceBetweenCenters - bulletRadius) / distanceBetweenCenters * scalarToPushToEdgeOfEllipse;
                            ...
                            const collisionX = fish.position[0] + scalarToShrinkToEdgeOfEllipse * vectorToBullet[0];
                            const collisionY = fish.position[1] + scalarToShrinkToEdgeOfEllipse * vectorToBullet[1];
                            const collisionPosition = [collisionX, collisionY];
