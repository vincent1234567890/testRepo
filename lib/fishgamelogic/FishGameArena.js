...
(function(root, factory) {
    ...
    function FishGameArena (onServer, gameConfig, fishGame, _gameStartTime) {
        ...
        function removeFish (fishId) {
            const fishClass = getFishClass(getFish(fishId).type);
            if (fishClass.tier === 2) {
                numberOfGoldFish--;
            }
        ...
        function considerSpawningFish () {
            ...
            if (fishes.length < gameConfig.maxFishOnScreen) {
                ...
                while (true) {
                    ...
                    fishClass = getFishClass(fishType);
                    if (fishClass.tier === 2 && numberOfGoldFish >= gameConfig.maximumGoldFish) {
                        //continue;
                    } else {
                        break;
                    }
        ...
        function handleCollision (bullet, collisionFish, explosionPosition) {
            const gunClass = getGunClass(bullet.gunId);
            ...
            for (let fishId in fishes) {
                ...
                if (distanceSquared < maxDist * maxDist) {
                    ...
                    if (damageDealt > 0) {
                        ...
                        if (fish.health <= 0) {
                            const fishValue = getFishClass(fish.type).value;
