        ...
        function checkCollisions () {
            ...
            for (let fishId in fishes) {
                ...
                for (let bulletId in bullets) {
                    ...
                    const maxDist = fishRadius + bulletRadius;
                    const distX = bullet.position[0] - fish.position[0];
                    const distY = bullet.position[1] - fish.position[1];

                    if (distX * distX + distY * distY < maxDist * maxDist) {
                        // Collision!
                        console.log(`Bullet ${bullet.id} hit fish ${fish.id}!`);

                        // @todo Inform clients of explosion
                        // @todo Harm fishes in splash radius
                        // @todo Inform clients of damage to fishes
                        // @todo If any fish dies, award player for the frag
