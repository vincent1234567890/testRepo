    ...
    const clientInformer = function (ioSocket) {
        ...
        return {
            ...
            bulletExploded: function (bulletId, position) {
                ioSocket.emit('x', {b: bulletId, l: position});   // client should already know the bullet's class and owner
            },

            fishAppeared: function (fishType, fishId, position, angle, when, motionPatternGroup, motionPatternId) {
                ioSocket.emit('f', {t: fishType, f: fishId, l: position, a: angle, w: when, mg: motionPatternGroup, mi: motionPatternId});
            },
