...
(function(root, factory) {
    ...
    function FishGameArenaServer (onServer, gameConfig, fishGame, _gameStartTime) {
        ...
        function getState () {
            return {
                gameTime: superArena.getGameTime(),
                fishes: superArena.getFishes(),
                bullets: superArena.getBullets(),
            };
        }
        ...
        function considerSpawningFish () {
            ...
            const fishes = superArena.getFishes();
            ...
            if (fishes.length < gameConfig.maxFishOnScreen) {
                ...
                //const fishClass = superArena.getFishClass(fishType);

                let fishType;
                let fishClass;
                while (true) {
                    fishType = chooseRandomFishType();
                    fishClass = superArena.getFishClass(fishType);
                ...
                for (let i = 0; i < numInGroup; i++) {
                    ...
                    const fishSpawnTime = superArena.getGameTime();
        ...
        function doUpdate () {
            const lastTickDeltaTime = superArena.updateEverything();
        ...
        function checkCollisions () {
            ...
            const fishes = superArena.getFishes();
            const bullets = superArena.getBullets();
            ...
            for (let bulletId in bullets) {
                ...
                for (let fishId in fishes) {
                    const fish = fishes[fishId];
                    const fishClass = superArena.getFishClass(fish.type);
        ...
        function handleCollision (bullet, collisionFish, explosionPosition) {
            const gunClass = superArena.getGunClass(bullet.gunId);
            ...
            for (let fishId in fishes) {
                ...
                if (distanceSquared < maxDist * maxDist) {
                    ...
                    if (damageDealt > 0) {
                        ...
                        if (fish.health <= 0) {
                            const fishValue = superArena.getFishClass(fish.type).value;
        ...
        const fishGameArenaServer = {
            getState: getState,
            changePlayerScore: changePlayerScore,
            closeArena: closeArena,
        };
        const self = fishGameArenaServer;
        return Object.assign(fishGameArenaServer, superArena, fishGameArenaServer);
    }
