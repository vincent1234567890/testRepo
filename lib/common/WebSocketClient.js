...
(function(root, factory) {
    ...
    proto.callAPIOnce = function (serviceName, funcName, data) {
        var Prom = Q && Q.Promise || Promise;
        return new Prom((resolve, reject) => {
            var APIError = Error;

            var service = this.getService(serviceName, true);
            if (!service) {
                throw APIError("Invalid service name: " + serviceName);
            }
            var wsFunc = service[funcName];
            if (!wsFunc) {
                throw APIError("Invalid func name: " + funcName);
            }
            ...
            if (wsFunc.isSync) {
                ...
                wsFunc.onceSync(key, function (res) {
                    if (res && res.status == 200) {
                        ...
                        reject(Error(res.errorMessage || res.error || JSON.stringify(res)));
                    }
                });
            }
            else {
